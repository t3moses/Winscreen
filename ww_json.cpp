 ...
 * Don't forget to delete "_ and _".
 * 
 */
...
// Calculate the string representation of the tip of the component vector that represents course and speed according to Seatalk.

  s_cr_x =  (String)(int16_t)( PIXELS_PER_KNOT * ww_vector::x_component_from_radial( ww_vector::x_rotate( p_var_display_data->xd_sh, d_hrr )).x );
  s_cr_y =  (String)(int16_t)( -PIXELS_PER_KNOT * ww_vector::x_component_from_radial( ww_vector::x_rotate( p_var_display_data->xd_sh, d_hrr )).y );

// Calculate the string representation of the tip of the component vector that represents course and speed according to GNSS.

  s_gr_x =  (String)(int16_t)( PIXELS_PER_KNOT * ww_vector::x_component_from_radial( ww_vector::x_rotate( p_var_display_data->xd_gh, d_hrr )).x );
  s_gr_y =  (String)(int16_t)( -PIXELS_PER_KNOT * ww_vector::x_component_from_radial( ww_vector::x_rotate( p_var_display_data->xd_gh, d_hrr )).y );
...
// relative to centre of the panel.
  ...
  if( p_var_display_data->d_gh > GH_BAND / 2.0 ) p_var_display_data->d_gh = GH_BAND / 2.0;
  else if( p_var_display_data->d_gh < -GH_BAND / 2.0 ) p_var_display_data->d_gh = -GH_BAND / 2.0;
  ...
  s_crh = (String)(int16_t)(( (double)PANEL_WIDTH / GH_BAND ) * p_var_display_data->d_gh );
...
// size_t outputCapacity;
  ...
  JsonObject st = var_01["st"].to<JsonObject>();
  st["x"] = s_gr_x;
  st["y"] = s_gr_y;
...
//
// s_crh is the string representation of d_gh, which is the canvas_4 'x' coordinate
// of the centre of the indicator bar (pixels).
//
...
//
  ...
  double d_gh = p_var_display_data->d_gh;

  if( d_arh > AH_BAND / 2.0 ) d_arh = AH_BAND / 2.0;
  else if( d_arh < -AH_BAND / 2.0 ) d_arh = -AH_BAND / 2.0;

  if( d_gh > GH_BAND / 2.0 ) d_gh = GH_BAND / 2.0;
  else if( d_gh < -GH_BAND / 2.0 ) d_gh = -GH_BAND / 2.0;

  String s_arh = (String)(int16_t)(( (double)P2_PANEL_WIDTH / ( 2 * AH_BAND )) * d_arh );
  String s_crh = (String)(int16_t)(( (double)P2_PANEL_WIDTH / ( 2 * GH_BAND )) * d_gh );
...
char ac_hrm[ 7 ];
  ...
  sprintf( ac_dev, "%+3.1f", p_var_display_data->d_gh );
  sprintf( ac_hrm, "%+3.1f", p_var_display_data->d_hm );
